'use client';

import { useState } from 'react';
import { loginComMagicLink, loginComGoogle } from './actions';
import Link from 'next/link';

export default function PaginaLogin() {
  const [mensagem, setMensagem] = useState<string | null>(null);
  const [erro, setErro] = useState<string | null>(null);
  const [carregando, setCarregando] = useState(false);

  async function handleMagicLink(formData: FormData) {
    setCarregando(true);
    setErro(null);
    const resultado = await loginComMagicLink(formData);
    if ('erro' in resultado && resultado.erro) {
      setErro(resultado.erro);
    } else if ('sucesso' in resultado && resultado.sucesso) {
      setMensagem(resultado.sucesso);
    }
    setCarregando(false);
  }

  async function handleGoogle() {
    setCarregando(true);
    setErro(null);
    const resultado = await loginComGoogle();
    if ('erro' in resultado && resultado.erro) {
      setErro(resultado.erro);
      setCarregando(false);
    } else if ('url' in resultado && resultado.url) {
      window.location.href = resultado.url;
    }
  }

  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <Link href="/" className="text-2xl font-bold text-emerald-600">
            MVP Suplementos
          </Link>
          <p className="mt-2 text-sm text-zinc-500">
            Entre para receber recomendações personalizadas
          </p>
        </div>

        <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          <form action={handleMagicLink} className="flex flex-col gap-4">
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-zinc-700 mb-1">
                Email
              </label>
              <input
                id="email"
                name="email"
                type="email"
                required
                placeholder="seu@email.com"
                className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
              />
            </div>

            <button
              type="submit"
              disabled={carregando}
              className="w-full rounded-lg bg-emerald-600 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition-colors disabled:opacity-50"
            >
              {carregando ? 'Enviando...' : 'Enviar link mágico'}
            </button>
          </form>

          <div className="relative my-4">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-zinc-200" />
            </div>
            <div className="relative flex justify-center text-xs">
              <span className="bg-white px-2 text-zinc-400">ou</span>
            </div>
          </div>

          <button
            onClick={handleGoogle}
            disabled={carregando}
            className="w-full rounded-lg border border-zinc-300 bg-white py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
          >
            <svg className="h-4 w-4" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            Continuar com Google
          </button>

          {mensagem && (
            <div className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">
              {mensagem}
            </div>
          )}
          {erro && <div className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{erro}</div>}
        </div>

        <p className="mt-4 text-center text-xs text-zinc-400">
          Ao entrar, você concorda com nossa{' '}
          <Link href="/politica-de-privacidade" className="underline hover:text-zinc-600">
            Política de Privacidade
          </Link>
          .
        </p>
      </div>
    </div>
  );
}
